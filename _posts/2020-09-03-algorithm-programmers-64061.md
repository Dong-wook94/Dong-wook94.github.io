...
public class Solution {
    ...
    public int solution(int[][] board, int[] moves) {
        ...
        for(int move : moves){
            moveCrain(move-1,board);
        }
        return answer;
    }

    private void moveCrain(int col,int[][] board) {
        for(int i=0;i<board.length;i++){
            int doll = board[i][col];
            if(doll==0) //빈공
                continue;
            if(!stack.empty() && stack.peek() == doll){//스택이 비지 않았고 스택 최상단이 같은경우
                stack.pop();
                answer += 2;
            }
            else{
...
이문제는 벌써 한 3번쯤 풀게되는 것 같다. java로는 처음이지만... 

점점더 코드또한 간결해 지는 것 같다. 
